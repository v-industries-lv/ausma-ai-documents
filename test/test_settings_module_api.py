...
class SettingsModuleAPITest(unittest.TestCase):
    def setUp(self):
        if os.path.exists('settings/mock_current.conf'):
            os.remove('settings/mock_current.conf')
        http.request("GET", f"http://{HOST}:{PORT}/config/restore_default_settings")

    def test_list_config(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/config").data.decode('utf-8')
        )
    ...
    def test_get_specific_config(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/config/default_system_prompt").data.decode('utf-8')
        )
        with open("settings/mock_initialized_default.conf", "r") as f:
            default_config = json.load(f)
    ...
    def test_set_specific_config(self):
        http.request("POST", f"http://{HOST}:{PORT}/config/default_system_prompt", json="New system prompt")
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/config/default_system_prompt").data.decode('utf-8')
        )
    ...
    def test_restore_defaults(self):
        ...
        http.request("POST", f"http://{HOST}:{PORT}/config/llm_runners", json=new_llm_runner)
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/config").data.decode('utf-8')
        )
    ...
    def test_set_system_prompt(self):
        ...
        http.request("POST", f"http://{HOST}:{PORT}/config/default_system_prompt", json=new_system_prompt)
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/config").data.decode('utf-8')
        )
