...
from test.mock_classes import MockKBService, MockKBStore, MockDocSource, MockLLMRunner, MockKnowledgeBase
...
class KBServiceAPITest(unittest.TestCase):
    def test_control_api(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb_service/control/start").data.decode('utf-8')
        )
        self.assertEqual(response, {'command': 'start'})
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb_service/control/stop").data.decode('utf-8')
        )
        self.assertEqual({'command': 'stop'}, response)

    def test_service_status(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb_service/status").data.decode('utf-8')
        )
        self.assertEqual({'status': 'test'}, response)

    def test_kb_list(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb").data.decode('utf-8')
        )
        self.assertEqual([{'name': 'mock1'}, {'name': 'mock2'}, {'name': 'mock3'}], response)

    def test_kb_config(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb/mock1/config").data.decode('utf-8')
        )
        self.assertEqual({'name': 'mock1'}, response)

    def test_kb_status(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb/mock1/status").data.decode('utf-8')
        )
    ...
    def test_kb_put(self):
        ...
        http.request("POST", f"http://{HOST}:{PORT}/kb/put", json=kb_config)
        print(http.request('GET', f"http://{HOST}:{PORT}/kb/new_kb/config").data.decode('utf-8'))
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb/new_kb/config").data.decode('utf-8')
        )
        print(response)
    ...
    def test_kb_delete(self):
        ...
        self.assertEqual(response, kb_config)
        response = json.loads(http.request('GET', f"http://{HOST}:{PORT}/kb/new_kb/delete").data.decode('utf-8'))
        self.assertEqual({"status": "success", "text": "Knowledge base deleted!"}, response)

    def test_doc_path(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/doc/**").data.decode('utf-8')
        )
        expected = [
    ...
    def test_doc_sources(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/doc_sources").data.decode('utf-8')
        )
        self.assertEqual({'name': 'test_name', 'type': 'test_type'}, response)

    def test_kb_tesseract_languages(self):
        response = json.loads(
            http.request('GET', f"http://{HOST}:{PORT}/kb/tesseract_languages").data.decode('utf-8')
        )
