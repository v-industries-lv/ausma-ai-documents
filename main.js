...
async function createWindow(url) {
  session.defaultSession.cookies.get({ url }).then((cookies) => {
    ...
    if (!hasUsername) {
      session.defaultSession.cookies.set({ url, name: 'username', value: os.userInfo().username });
    }
  });
...
async function launchLocalServer() {
  if (fs.existsSync("flask_app/flask_app")) {
    ...
    serverProcess = spawn('flask_app/flask_app', ['--production', '--localhost']);
  } else {
    // sets the path to include the virtual python environment
    let env = Object.assign({}, process.env);
    env["PATH"] = __dirname + "/.venv/bin:" + env["PATH"];
    serverProcess = spawn('python3', ['flask_app.py', '--production', '--localhost'], { env: env });
  }
