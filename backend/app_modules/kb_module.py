...
class KBModule:
    ...
    def parse(kb_config: dict):
        ...
        try:
            ...
            key_types = {"name": str, "selection": list, "convertors": list, "embedding": dict}
                ...
                if convertor["conversion"] in ["ocr_llm", "llm"]:
                    ...
                    for key, value in convertor.items():
                        if key not in allowed_key_types.keys():
                            logger.error(
                                f"Convertor key \"{key}\" is not allowed! Expected: {allowed_key_types.keys()}")
                            return False
                        if not isinstance(convertor[key], allowed_key_types[key]):
                            logger.error(
                                f"Convertor key \"{key}\" value cannot be {type(convertor[key])}! Expected: {allowed_key_types[key]}")
                            return False
    ...
    def apply_routes(self, app: Flask):
        # KB service
        @app.route('/api/kb_service/control/start')
        def kb_service_start():
            self.kb_service.start()
            return {"command": "start"}

        @app.route('/api/kb_service/control/stop')
        def kb_service_stop():
            self.kb_service.stop()
            return {"command": "stop"}

        @app.route('/api/kb_service/status')
        ...
        @app.route('/api/kb/put', methods=['POST'])
        def kb_put():
            raw_kb_config = request.get_json()
            kb_config = KBModule.parse(raw_kb_config)
            if KBModule.validate_kb_config(kb_config) and self.kb_service.kb_store.upsert(kb_config):
        ...
        @app.route('/api/kb/<path:name>/delete', methods=['POST'])
        def kb_delete(name):
            if self.kb_service.kb_store.delete(name):
                return app.response_class(
                ...
                return app.response_class(
                    response=json.dumps({"status": "error", "text": f"Failed to delete knowledge base {name}!"},
                                        indent=2),
                    mimetype='application/json'
                )

        @app.route('/api/kb/<path:name>/clear', methods=['POST'])
        def kb_clear(name):
            kb = self.kb_service.kb_store.get(name)
            if kb.clear():
                return app.response_class(
                    response=json.dumps({"status": "success", "text": "Knowledge base cleared!"}, indent=2),
                    mimetype='application/json'
                )
            else:
                return app.response_class(
                    response=json.dumps({"status": "error", "text": f"Failed to clear knowledge base {name}!"},
                                        indent=2),
                    mimetype='application/json'
                )

        @app.route('/api/kb/<path:name>/config')
        def kb_config(name):
            kb = self.kb_service.kb_store.get(name)
            if kb is None:
                return app.response_class(
        ...
        @app.route('/api/kb/<path:name>/status')
        def kb_status(name):
        ...
        @app.route('/api/kb/tesseract_languages')
        def kb_languages():
        ...
        def docs_all():
            return app.response_class(
                response=json.dumps(sorted(self.kb_service.doc_source.list_items('*'), key=lambda x: x["path"]),
                                    indent=2),
                mimetype='application/json'
            )
        ...
        def doc_path(path):
            return app.response_class(
                response=json.dumps(sorted(self.kb_service.doc_source.list_items(path), key=lambda x: x["path"]),
                                    indent=2),
                mimetype='application/json'
            )

        @app.route('/api/doc_sources')
        def doc_sources():
            return app.response_class(
