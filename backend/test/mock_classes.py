...
from kb.knowledge_base import KBStore, KnowledgeBase
...
class MockKnowledgeBase(KnowledgeBase):
    ...
    @staticmethod
    def from_dict(kb_dict):
        return MockKnowledgeBase(kb_dict)

    @staticmethod
    def create(name: str, selection: List[str], convertor_configs: List[dict],
               embedding_config: dict):
        return MockKnowledgeBase(
            {"name": name, "selection": selection, "convertors": convertor_configs, "embedding": embedding_config})
...
class MockKBStore(KBStore):
    ...
    def get(self, name: str) -> Optional[KnowledgeBase]:
        kb_dict = self.kbs.get(name)
        if kb_dict is None:
            return None
        return MockKnowledgeBase(kb_dict)

    def list(self):
        return [MockKnowledgeBase(v) for v in self.kbs.values()]

    def upsert(self, kb_config: dict):
        self.kbs[kb_config["name"]] = kb_config
...
class MockChroma(VectorStore):
    ...
    def __init__(self, client, embedding_function, collection_name):
        self.client = client
    ...
    def get(self, **kwargs):
        if kwargs.get("where") is not None:
            ...
            if kwargs["where"].get("document_hash") is not None or kwargs["where"].get("output_hash"):
                ...
                if kwargs["where"].get("document_hash") == document_hash or kwargs["where"].get(
